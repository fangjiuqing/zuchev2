...
class my_module extends user_module {
    /**
     * 订单列表
    ...
    public function order_action () {
        ...
        $results = [];
        foreach ( $order_type as $ot => $tv ) {
            $results[$ot] = [];
            foreach ( $orders as $order ) {
                if ( $ot == $order['order_status'] ) {
                    $results[$ot][] = $order;
                }
            }
        }
        $this->assign('orders' , $results);
    ...
    public function view_action () {
        ...
        $order_info = $tab->get();
        $order_car_info = json_decode($order_info['order_car_info'] , true);
        //var_dump($order_info,$order_car_info);die;
