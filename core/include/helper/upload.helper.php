...
/**
 * 上传助手类
 * $Id: upload.helper.php 934 2017-12-11 17:09:37Z reginx $
 */
class upload_helper extends rgx {
     ...
     */
    public static function addrecord ($path, $size, $sname, $hash = '', $ext = 'unkown' ) {
        $obj = OBJ('attachment_table');
        //$data['attch_type']    =    $type;
        $data['attch_path']    =    $path;
        $data['attch_ext']     =    strtolower($ext);
        $data['attch_name']    =    $sname;
        $data['attch_size']    =    $size;
        $data['attch_hash']    =    $hash;
        $data['attch_adate']   =    REQUEST_TIME;
        if ( $obj->load($data) ) {
            $ret    =    $obj->save();
            if ( $ret['code'] == 0 ) {
                return $ret['row_id'];
