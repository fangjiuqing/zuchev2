 ...
 * @author Tanxu
 */
class geetest_helper extends rgx {

    const GT_SDK_VERSION = 'php_3.2.0';

    const PC_ID  = 'ee8599e5325da910ae7ef76154ec345c';

    const PC_KEY = '183dd0fe162d506df6b8debd84e7031f';

    const M_ID  = 'ee8599e5325da910ae7ef76154ec345c';

    const M_KEY = '183dd0fe162d506df6b8debd84e7031f';

    const U_ID  = 'reginx@qq.com';
     ...
     */
    public static function get_instance ($platform = 1) {
        $ret = null;
        if (1 == $platform) {
            $ret = new geetest_helper(self::PC_ID, self::PC_KEY);
        }
        else {
            $ret = new geetest_helper(self::M_ID, self::M_KEY);
        }
        return $ret;
    }
    ...
    public function captcha () {
        return [
            'alive'     => $this->pre_process(self::U_ID),
            'data'      => $this->get_response_str()
        ];
    }
     ...
     * @return [type] [description]
     */
    public function verify ($alive, $geetest_challenge, $geetest_validate, $geetest_seccode) {
        set_time_limit(0);
        $ret = false;
        if ($alive) {
            $ret = $this->success_validate($geetest_challenge, $geetest_validate, $geetest_seccode, self::U_ID);
        }
        else {
            $ret = $this->fail_validate($geetest_challenge, $geetest_validate, $geetest_seccode);
        }
        return $ret ? true : false;
    }

    private function __construct($captcha_id, $private_key) {
