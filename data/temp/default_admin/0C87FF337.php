...
<?php endif;?><div class="table-responsive">
                            <table class="table table-bordered">
                                ...
                                <tbody> <?php if (empty($list)):?> <tr>
                                    ...
                                    </tr> <?php endif;?><?php unset($k, $v); $k_index = 0; foreach ((array)$list as $k => $v): $k_index ++;?> <tr id="item-<?php echo($v['agent_id']);?>">
                                        <td>
                                            <input type="checkbox" class="i-checks" value="<?php echo($v['agent_id']);?>" />
                                        </td>

                                        <td>
                                            <a title="编辑<?php echo($v['agent_code']);?>" href="<?php echo(RGX\router::url('agent-add-id-%d' , $v['agent_id'])); ?>"><?php echo($v['agent_code']);?></a>
                                        </td>
                                        <td><?php echo($v['agent_fullname']);?></td>
                                        <td><?php echo($v['agent_mobile']);?></td>
                                        <td><?php echo($agent_type[$v['agent_level']]);?></td>
                                        <td><?php echo(intval($v['agent_order_nums']));?></td>
                                        <td class="text-warning">￥<?php echo(intval($v['agent_order_amount']));?></td>

                                        <td class="agent-operate">
                                            <a href="<?php echo(RGX\router::url('agent-del-id-%d' , $v['agent_id'])); ?>" data-confirm="操作不可撤销，确认吗？" data-target="#item-<?php echo($v['agent_id']);?>" class="btn btn-xs btn-danger btn-delete">
...
<script>
    ...
    <script src="http://localhost/zuchev2/admin/template/default/js/filter.js"></script>
    <script type="text/javascript">
